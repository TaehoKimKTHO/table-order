...
import type { RestaurantTable, TableSession, OrderWithItems, OrderItem } from '@/types';
        ...
        {loading ? (
          ...
          <div className="bg-surface rounded-[20px] border border-border shadow-[0_4px_12px_rgba(0,0,0,0.03)] overflow-hidden">
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {sessions.map((s) => {
                  const allItems: OrderItem[] = s.orders.flatMap((o) => o.items || []);
                  const table = tables.find((t) => t.id === s.session.table_id);
                  return (
                    <tr key={s.session.id} className="border-b border-border last:border-0 hover:bg-surface-alt">
                      ...
                      <td className="px-6 py-6">
                        <span className="inline-block bg-primary-bg text-primary px-2.5 py-1 rounded-md font-bold text-[13px]">
                          TABLE {table?.table_number || s.session.table_id}
                        </span>
                      </td>
                      <td className="px-6 py-6">
                        <div className="text-sm font-medium">
                          {formatTime(s.session.started_at)} ~ {s.session.ended_at ? formatTime(s.session.ended_at) : '-'}
                        </div>
                        <span className="inline-block mt-1.5 text-xs text-text-muted bg-bg px-2 py-0.5 rounded">
                          {calcDuration(s.session.started_at, s.session.ended_at)}
                        </span>
                      </td>
                      <td className="px-6 py-6">
                        ...
                        <div className="text-[13px] text-text-muted mt-2 leading-relaxed">
                          {allItems.slice(0, 3).map((item, i) => (
                            <span key={i}>{item.menu_name} x{item.quantity}{i < Math.min(allItems.length, 3) - 1 ? <br /> : ''}</span>
                          ))}
                          {allItems.length > 3 && <span><br />외 {allItems.length - 3}건</span>}
                        </div>
                      </td>
                      <td className="px-6 py-6 text-right">
                        <span className="text-lg font-extrabold text-text">{s.totalAmount.toLocaleString('ko-KR')}원</span>
                      </td>
                    </tr>
                  );
                })}
