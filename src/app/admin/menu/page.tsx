...
export default function MenuManagementPage() {
  ...
  return (
    <>
      ...
      <div className="flex-1 p-10 overflow-y-auto">
          ...
          <button
            ...
            className={`px-5 py-2.5 rounded-[10px] font-semibold text-sm border transition-colors cursor-pointer ${
              selectedCategory === null
                ...
                : 'bg-surface text-text-muted border-border hover:bg-surface-alt'
            }`}
          >
            전체
          </button>
        ...
        <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-6">
          {filteredItems.map((item) => (
            <div
              key={item.id}
              className={`bg-surface rounded-[20px] border border-border shadow-[0_4px_12px_rgba(0,0,0,0.03)] overflow-hidden flex flex-col transition-transform hover:-translate-y-1 hover:shadow-[0_8px_24px_rgba(0,0,0,0.08)] ${
            ...
            >
              ...
              <div className={`h-40 bg-border flex items-center justify-center text-[40px] relative ${!item.isAvailable ? 'grayscale' : ''}`}>
                ...
                <span className={`absolute top-3 right-3 px-3 py-1 rounded-full text-xs font-extrabold backdrop-blur-[10px] ${
                  item.isAvailable
                    ...
                    : 'bg-status-wait/90 text-white'
                }`}>
                  {item.isAvailable ? '판매중' : '일시품절'}
                </span>
              </div>
              ...
              <div className="p-5 flex-1 flex flex-col">
                ...
                <div className="text-[13px] text-text-muted leading-snug flex-1">
                  {item.description}
                </div>
              </div>
              ...
              <div className="px-5 py-4 border-t border-border flex gap-3 bg-surface-alt">
                <button
                  ...
                  className="flex-1 py-2.5 rounded-lg border border-border bg-surface text-text font-semibold text-[13px] cursor-pointer hover:bg-bg transition-colors"
                >
                  정보 수정
                </button>
                <button
                  onClick={() => handleToggleAvailable(item)}
                  className={`flex-1 py-2.5 rounded-lg border border-border bg-surface font-semibold text-[13px] cursor-pointer hover:bg-bg transition-colors ${
                    item.isAvailable ? 'text-primary' : 'text-text'
                  }`}
                >
      ...
      {showForm && (
        <MenuFormModal categories={categories} editingItem={editingItem} onClose={handleFormClose} />
      )}
      {showCategoryManager && (
        <CategoryManager categories={categories} onClose={() => { setShowCategoryManager(false); fetchData() }} />
      )}
    </>
  )
}
